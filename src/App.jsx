...
const MUSCOLI = ["Pettorali","Dorsali","Spalle","Bicipiti","Tricipiti","Avambracci","Addominali","Obliqui","Lombari","Glutei","Quadricipiti","Femorali","Polpacci","Tibiali","Core","Diaframma","Cardiovascolare"];
...
<button key={id} onClick={() => { setMode(id); setMsg(""); setRuolo("cliente"); }} style={{ flex: 1, padding: "8px 0", borderRadius: 8, border: "none", cursor: "pointer", fontWeight: 600, fontSize: 13, background: mode===id ? C.surface : "transparent", color: mode===id ? C.text : C.muted, fontFamily: "inherit" }}>{la}</button>
))}
</div>
{mode === "register" && (
...
const [nuovoEx, setNuovoEx] = useState({ nome: "", tipo: "", muscoli: "", muscoli_secondari: "", attrezzi: "", difficoltà: "", descrizione: "", video_url: "" });
...
<select value={nuovoEx.difficoltà} onChange={e => setNuovoEx(p => ({...p, difficoltà: e.target.value}))}
style={{ width: "100%", padding: "11px 14px", borderRadius: 10, border: `1.5px solid ${C.border}`, fontSize: 14, fontFamily: "inherit", background: "#fff" }}>
...
const { data, error } = await supabase.from("esercizi").insert([nuovoEx]).select().single();
if (error) { alert("Errore: " + error.message); return; }
if (data) {
setEsercizi(prev => [...prev, data]);
setNuovoEx({ nome: "", tipo: "", muscoli: "", muscoli_secondari: "", attrezzi: "", difficoltà: "", descrizione: "", video_url: "" });
...
<div style={{ fontSize: 12, color: C.muted, marginBottom: 6 }}>{e.tipo} · {e.difficoltà}</div>
...
s.textContent = `* { box-sizing: border-box; margin: 0; padding: 0; } body { font-family: sans-serif; } select, option { color: #0D1F17 !important; background: white !important; }`;
