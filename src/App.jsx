...
const Card = ({ children, style = {} }) => (
  <div style={{ background: C.surface, borderRadius: 16, boxShadow: C.shadow, padding: 20, border: `1px solid ${C.border}88`, ...style }}>
    {children}
  </div>
);
...
const Btn = ({ children, onClick, variant = "primary", full = false, style = {} }) => {
  const v = {
    ...
    dark: { background: C.primary, color: "#fff" },
    ghost: { background: "transparent", color: C.primary, border: `1.5px solid ${C.border}` },
  };
  return (
    <button onClick={onClick} style={{ border: "none", borderRadius: 10, fontFamily: "inherit", fontWeight: 600, padding: "11px 20px", fontSize: 14, display: "inline-flex", alignItems: "center", justifyContent: "center", gap: 6, width: full ? "100%" : "auto", cursor: "pointer", transition: "all 0.15s", ...v[variant], ...style }}>
      {children}
    </button>
  );
};
...
const Input = ({ label, type = "text", value, onChange, placeholder }) => (
  <div style={{ marginBottom: 16 }}>
    ...
    <input type={type} value={value} onChange={onChange} placeholder={placeholder}
      style={{ width: "100%", padding: "11px 14px", borderRadius: 10, border: `1.5px solid ${C.border}`, fontSize: 14, outline: "none", fontFamily: "inherit", color: C.text }} />
  </div>
);
...
function LoginScreen({ onLogin }) {
        ...
        <Card style={{ padding: 32 }}>
          <div style={{ display: "flex", gap: 4, marginBottom: 24, background: C.bg, padding: 4, borderRadius: 10 }}>
            {[["login","Accedi"],["register","Registrati"]].map(([id,la]) => (
              <button key={id} onClick={() => setMode(id)} style={{ flex: 1, padding: "8px 0", borderRadius: 8, border: "none", cursor: "pointer", fontWeight: 600, fontSize: 13, background: mode===id ? C.surface : "transparent", color: mode===id ? C.text : C.muted, fontFamily: "inherit" }}>{la}</button>
            ))}
          </div>
          {mode === "register" && (
            <>
  ...
  useEffect(() => {
    supabase.from("pazienti").select("*").then(({ data }) => data && setPazienti(data));
    ...
    <div style={{ display: "flex", height: "100vh", fontFamily: "sans-serif", background: C.bg }}>
      <div style={{ width: 224, background: C.primary, display: "flex", flexDirection: "column" }}>
        ...
        <nav style={{ padding: "14px 10px", flex: 1, display: "flex", flexDirection: "column", gap: 3 }}>
          {[["dashboard","⬡","Dashboard"],["clienti","👥","Clienti"],["esercizi","🏋️","Esercizi"],["chat","💬","Chat"]].map(([id,ic,la]) => (
      ...
      <div style={{ flex: 1, overflow: "auto", padding: 32 }}>
        {view === "dashboard" && (
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: 14, marginBottom: 28 }}>
              {[["👥", pazienti.length, "Clienti registrati", C.accent],["🏋️", esercizi.length, "Esercizi in libreria", C.orange]].map(([ic,v,la,c]) => (
            ...
            <Card style={{ padding: 22 }}>
              ...
              {pazienti.length === 0 && <p style={{ color: C.muted, fontSize: 14 }}>Nessun cliente ancora registrato.</p>}
              {pazienti.map(p => (
                <div key={p.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "10px 0", borderBottom: `1px solid ${C.border}44` }}>
                  <div style={{ width: 36, height: 36, borderRadius: 18, background: C.accentSoft, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 18 }}>👤</div>
                  <div style={{ flex: 1 }}>
                    <div style={{ fontWeight: 600, fontSize: 14 }}>{p.nome} {p.cognome}</div>
                    <div style={{ fontSize: 12, color: C.muted }}>{p.email} · {p.ruolo}</div>
                  </div>
                </div>
              ))}
            </Card>
        ...
        {view === "clienti" && (
          <div>
            ...
            {pazienti.length === 0 && <p style={{ color: C.muted }}>Nessun cliente ancora registrato.</p>}
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(300px,1fr))", gap: 14 }}>
              {pazienti.map(p => (
                <Card key={p.id} style={{ padding: 20 }}>
                  <div style={{ fontWeight: 700, fontSize: 16 }}>{p.nome} {p.cognome}</div>
                  <div style={{ fontSize: 13, color: C.muted, marginTop: 4 }}>{p.email}</div>
                  <div style={{ marginTop: 8, fontSize: 12, color: C.muted }}>📱 {p.telefono || "—"} · ⚖️ {p.peso || "—"} kg</div>
                  {p.condizioni && <div style={{ marginTop: 10, padding: "8px 12px", background: "#FFF8F0", borderRadius: 8, fontSize: 12, color: "#8A4B1A" }}>⚠️ {p.condizioni}</div>}
                </Card>
              ))}
            </div>
          </div>
        ...
        {view === "esercizi" && (
          <div>
            ...
            {esercizi.length === 0 && <p style={{ color: C.muted }}>Nessun esercizio ancora inserito.</p>}
            ...
            {pazienti.map(p => (
              <Card key={p.id} style={{ padding: 18, marginBottom: 10 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                  <div style={{ width: 40, height: 40, borderRadius: 20, background: C.accentSoft, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>👤</div>
                  <div>
  ...
  const saveProfilo = async () => {
    await supabase.from("pazienti").update(profilo).eq("email", user.email);
  ...
  return (
    <div style={{ maxWidth: 480, margin: "0 auto", minHeight: "100vh", background: C.bg, fontFamily: "sans-serif" }}>
      <div style={{ padding: 20, paddingBottom: 80 }}>
        {view === "home" && (
          <div>
            <h1 style={{ fontSize: 24, fontWeight: 800, color: C.text, marginBottom: 6 }}>Ciao, {user.nome}! 👋</h1>
            ...
            <Card style={{ background: `linear-gradient(135deg, ${C.primary}, #2D6E52)`, marginBottom: 20 }}>
              <div style={{ color: "#fff" }}>
                ...
                <div style={{ fontWeight: 700, fontSize: 16 }}>
                  {user.ruolo === "paziente_eb" ? "🩺 Percorso EB BodyMind" : "💪 Percorso Allenamento"}
                </div>
                <div style={{ fontSize: 12, opacity: 0.65, marginTop: 4 }}>Il tuo trainer sta preparando la tua scheda</div>
              </div>
            </Card>
            <Card style={{ padding: 20 }}>
              <h3 style={{ fontWeight: 700, fontSize: 15, marginBottom: 12 }}>📋 La tua scheda</h3>
              <p style={{ fontSize: 14, color: C.muted, lineHeight: 1.6 }}>La tua scheda di allenamento personalizzata sarà disponibile qui non appena il tuo trainer la creerà.</p>
            </Card>
          </div>
        )}
        {view === "profilo" && (
          <div>
            ...
            <Card style={{ padding: 20, marginBottom: 14 }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <textarea value={profilo.condizioni || ""} onChange={e => setProfilo({...profilo, condizioni: e.target.value})}
                  placeholder="Descrivi ferite, dolori, operazioni o qualsiasi condizione..." rows={4}
                  style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: `1.5px solid ${C.border}`, fontSize: 13, resize: "none", outline: "none", fontFamily: "inherit" }} />
              </div>
              <Btn full variant="dark" onClick={saveProfilo} style={{ borderRadius: 10, padding: 12 }}>💾 Salva Profilo</Btn>
            </Card>
            <Btn full variant="ghost" onClick={onLogout} style={{ borderRadius: 10, padding: 12, color: C.red, borderColor: C.red + "88" }}>← Esci</Btn>
          </div>
        )}
        {view === "chat" && (
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 800, marginBottom: 20 }}>Chat con il Trainer</h1>
            <Card style={{ padding: 20 }}>
              <p style={{ fontSize: 14, color: C.muted }}>La chat sarà disponibile a breve! 💬</p>
            </Card>
          </div>
        )}
      </div>
