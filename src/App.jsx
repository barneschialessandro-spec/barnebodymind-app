        ...
        <nav style={{ padding: "14px 10px", flex: 1, display: "flex", flexDirection: "column", gap: 3 }}>
          {[["dashboard","⬡","Dashboard"],["clienti","👥","Clienti"],["esercizi","🏋️","Esercizi"],["chat","💬","Chat"],["impostazioni","⚙️","Impostazioni"]].map(([id,ic,la]) => (
            <button key={id} onClick={() => setView(id)}
              style={{ display: "flex", alignItems: "center", gap: 10, padding: "10px 14px", borderRadius: 10, border: "none", cursor: "pointer", background: view===id ? "rgba(61,201,138,0.15)" : "transparent", color: view===id ? C.accent : "rgba(255,255,255,0.6)", fontWeight: view===id ? 600 : 400, fontSize: 14, fontFamily: "inherit", textAlign: "left" }}>
              <span>{ic}</span>{la}
            </button>
          ))}
        </nav>
                ...
                <Card key={p.id} style={{ padding: 20 }}>
                  <div style={{ display: "flex", gap: 14 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <div style={{ fontSize: 13, color: C.muted, marginTop: 2 }}>{p.email}</div>
                      <div style={{ fontSize: 12, color: C.muted, marginTop: 4 }}>📱 {p.telefono || "—"} · ⚖️ {p.peso || "—"} kg</div>
              ...
              {selCliente ? (
                <>
                  ...
                  <div style={{ flex: 1, overflow: "auto", padding: "20px 22px", display: "flex", flexDirection: "column", gap: 12 }}>
                    {msgs.map((m,i) => (
                      <div key={i} style={{ display: "flex", justifyContent: m.da_utente===user.email ? "flex-end" : "flex-start" }}>
                        <div style={{ maxWidth: "65%", padding: "10px 14px", borderRadius: m.da_utente===user.email ? "16px 4px 16px 16px" : "4px 16px 16px 16px", background: m.da_utente===user.email ? C.primary : C.surface, color: m.da_utente===user.email ? "#fff" : C.text, boxShadow: C.shadow, fontSize: 14 }}>
                          {m.testo}
                  ...
                  <div style={{ padding: "14px 22px", borderTop: `1px solid ${C.border}`, display: "flex", gap: 10 }}>
                    <input value={newMsg} onChange={e => setNewMsg(e.target.value)} onKeyDown={e => e.key==="Enter" && sendMsg()} placeholder="Scrivi un messaggio..."
                      style={{ flex: 1, padding: "11px 16px", borderRadius: 12, border: `1.5px solid ${C.border}`, fontSize: 14, outline: "none" }} />
        ...
        {view === "chat" && (
          <div style={{ display: "flex", flexDirection: "column", height: "calc(100vh - 80px)" }}>
            ...
            <div style={{ flex: 1, overflow: "auto", display: "flex", flexDirection: "column", gap: 12, paddingBottom: 12 }}>
              ...
              {msgs.map((m,i) => (
                <div key={i} style={{ display: "flex", justifyContent: m.da_utente===user.email ? "flex-end" : "flex-start" }}>
                  <div style={{ maxWidth: "78%", padding: "10px 14px", borderRadius: m.da_utente===user.email ? "16px 4px 16px 16px" : "4px 16px 16px 16px", background: m.da_utente===user.email ? C.primary : C.surface, color: m.da_utente===user.email ? "#fff" : C.text, boxShadow: C.shadow, fontSize: 14 }}>
                    {m.testo}
                    <div style={{ fontSize: 10, opacity: 0.55, marginTop: 4, textAlign: "right" }}>{m.ora}</div>
                  </div>
                </div>
            ...
            <div style={{ display: "flex", gap: 8, paddingTop: 8 }}>
              <input value={newMsg} onChange={e => setNewMsg(e.target.value)} onKeyDown={e => e.key==="Enter" && sendMsg()} placeholder="Scrivi al tuo trainer..."
                style={{ flex: 1, padding: "12px 16px", borderRadius: 24, border: `1.5px solid ${C.border}`, fontSize: 14, outline: "none", background: C.bg }} />
            ...
            <Card style={{ padding: 20, marginBottom: 14 }}>
              ...
              <div style={{ marginBottom: 16 }}>
                ...
                <textarea value={profilo.condizioni || ""} onChange={e => setProfilo({...profilo, condizioni: e.target.value})}
                  placeholder="Descrivi ferite, dolori, operazioni..." rows={5}
